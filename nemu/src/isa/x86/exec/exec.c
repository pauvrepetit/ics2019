...
make_group(gp2,
    ...
    IDEX(nop, shl), IDEX(nop, shr), EMPTY, IDEX(nop, sar))

/* 0xf6, 0xf7 */
make_group(gp3,
    IDEX(test_I, test), EMPTY, IDEX(nop, not), EMPTY,
    IDEX(nop, mul), IDEX(nop, imul1), EMPTY, IDEX(nop, idiv))
...
/* 0xff */
make_group(gp5,
    IDEX(inc_rm, inc), EMPTY, IDEX(nop, call_rm), EMPTY,
    IDEX(nop, jmp_rm), EMPTY, IDEX(push_m, push), EMPTY)
...
static OpcodeEntry opcode_table [512] = {
  ...
  /* 0xe8 */	IDEX(call_rel, call), EMPTY, EMPTY, IDEXW(jmp_imm8, jmp, 1),
...
/*2 byte_opcode_table */
  ...
  /* 0xb4 */	EMPTY, EMPTY, IDEX(movzb, movzx), IDEXW(mov_E2G, movzx, 2),
